...
bool zero(){
    ...
    Serial.println(F(" Success Translation is "));
    Serial.println(success_translation);
    ...
    Serial.println(F(" Success Rotation is "));
    Serial.println(success_rotation);
...
bool zeroRotation(){
    Serial.println(F("ENTERED THE ZERO ROATATITOANTIAOTNTONTONTOANTONTONTNTaonNTIOAinOIANTON"));
    for (int i = 0; i<SENSOR_DEBOUNCE*2; i++) rotational_zero.sensorMonitor();
    int timer_serial = 0;
    int timer_bounce = 0;
    int count = 0;
    
    rotation_stepper.setSpeedInMillimetersPerSecond(HOME_SPEED_ROTATION);    
    ...
    while(digitalRead(ROTATION_DRIVER_ZERO)==HIGH){
        ...
        rotation_stepper.moveRelativeInMillimeters(5); //MOVES 
 ...
 //   Serial.println(translation_stepper.getCurrentPositionInMillimeters());
    ...
    while(digitalRead(ROTATION_DRIVER_ZERO)==LOW){
        ...
        rotation_stepper.moveRelativeInMillimeters(-1);
    }
    
    
     while(digitalRead(ROTATION_DRIVER_ZERO)==HIGH){
        rotation_stepper.moveRelativeInMillimeters(0.5); //MOVES       
    }
    rotation_stepper.setCurrentPositionInMillimeters(0.00);
...
bool zeroTranslation(){
 ...
 //   Serial.println(translation_stepper.getCurrentPositionInMillimeters());
    ...
    while(digitalRead(TRANSLATION_DRIVER_ZERO)==LOW){
     ...
     while(digitalRead(TRANSLATION_DRIVER_ZERO)==HIGH){
        translation_stepper.moveRelativeInMillimeters(0.5); //MOVES       
    }
    translation_stepper.setCurrentPositionInMillimeters(0.00);
